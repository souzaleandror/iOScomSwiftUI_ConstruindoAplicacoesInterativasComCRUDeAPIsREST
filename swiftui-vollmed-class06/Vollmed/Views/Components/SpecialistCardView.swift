...
struct SpecialistCardView: View {
    ...
    func downloadImage() async {
        do {
            ...
            print("Ocorreu um erro ao oter a imagem: \(error)")
        }
    }
        ...
        VStack(alignment: .leading) {
            HStack(spacing: 16.0) {
                ...
                VStack(alignment: .leading, spacing: 8.0) {
                    ...
                    if let appointment {
                        Text(appointment.date.convertDateStringToReadableDate()).bold()
                    }
                }
            }
            ...
            if let appointment {
                HStack {
                    NavigationLink { ScheduleAppointmentView(specialistID: specialist.id, isRescheduleView: true,  appointmentID: appointment.id)}  label: {
